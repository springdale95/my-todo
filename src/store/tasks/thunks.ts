...
>('get/tasks', async (_, { dispatch }) => {
    try {
        const response = await axios.get(url);
        const showNotificationAction: NotificationState = {
            show: true,
            notificationText: 'Задачи загружены',
            type: 'success',
        };
...
export const fetchAddTask = createAsyncThunk<
    ...
    ITask,
    { state: RootState; dispatch: AppDispatch }
>('post/task', async (newTask: ITask, { dispatch }) => {
    try {
        const response = await axios.post(url, newTask);
        const showNotificationAction: NotificationState = {
            show: true,
            notificationText: 'Задача добавлена',
            type: 'success',
        };
        const hideNotificationAction: NotificationState = {
            show: false,
...
export const fetchChangeTaskStatus = createAsyncThunk<
    ITask,
    ITask,
    { state: RootState; dispatch: AppDispatch }
>('put/taskStatus', async (updatedTask: ITask, { dispatch }) => {
    try {
        const response = await axios.put(url + updatedTask.id, updatedTask);
        const showNotificationAction: NotificationState = {
            show: true,
            notificationText: 'Статус задачи обновлен',
            type: 'success',
        };
        const hideNotificationAction: NotificationState = {
            show: false,
...
export const fetchDeleteTask = createAsyncThunk<
    ...
    ITask,
    { state: RootState; dispatch: AppDispatch }
>('delete/task', async (task: ITask, { dispatch }) => {
    try {
        const response = await axios.delete(url + task.id);
        const showNotificationAction: NotificationState = {
            show: true,
            notificationText: 'Задача удалена',
            type: 'success',
        };
        const hideNotificationAction: NotificationState = {
            show: false,
...
export const fetchEditTask = createAsyncThunk<
    ...
    ITask,
    { state: RootState; dispatch: AppDispatch }
>('put/taskText', async (task: ITask, { dispatch }) => {
    try {
        const response = await axios.put(url + task.id, task);
        const showNotificationAction: NotificationState = {
            show: true,
            notificationText: 'Задача редактирована',
            type: 'success',
        };
        const hideNotificationAction: NotificationState = {
            show: false,
